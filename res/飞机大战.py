...
APP_ICON = "C:/Users/Rioron/PycharmProjects/untitled11/res/bili.ico"    # 设计公共的图片常量，定义在文件中，方便管理
IMG_BACKGROUND ="C:/Users/Rioron/PycharmProjects/untitled11/res/img_bg_level_2.jpg"   # 设计公共的图片常量，定义在文件中，方便管理
# 设置敌机图片库常量元组
...
class Model:
    ...
    def __init__(self,img_path,x,y):        #
        ...
        self.x = x  # 窗体中放置的x坐标
        self.y = y  # 窗体中放置的y坐标
        #window = pygame.display.set_mode(WINDOW_WIDTH, WINDOW_HEIGHT))# 将模型加入窗体的方法抽取到父类
    def display(self):
        self.window.blit(self.img, (self.x, self.y))  # 使用Model的类变量访问窗体对象  # 调用bilt方法，将图片加入到窗体中
#将图片传送到由，x,y组成的平面上
# 背景类
class Background(Model):
    ...
    def move(self):
        # 加入背景移动的情况判定
        if self.y <= Game.WINDOW_HEIGHT:    # 如果没有超出屏幕就正常移动  "                                        """TODO:WINDOW_HEIGHT??"""
            self.y += 1  # 纵坐标自增1
        else:   # 如果超出屏幕，恢复图片位置为原始位置
            self.y = 0  # 纵坐标 = 0

    # 覆盖父类display方法，制作原始背景贴图+辅助背景贴图
    def display(self):          #两张背景图连接，窗口只显示一张，当图片移动到窗口底端时恢复原来的位置
...
class Game:
    ...
    # 初始化窗体
    def frame_init(self):
        Model.window = pygame.display.set_mode((Game.WINDOW_WIDTH, Game.WINDOW_HEIGHT))  # 使用窗体常量替换原始格式  # 初始化窗体
